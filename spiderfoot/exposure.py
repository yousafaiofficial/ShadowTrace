...
class EntityResolver:
    """Resolves raw SpiderFoot events into structured entities and typed relationships."""
    ...
    def resolve_entities_and_relations(self, events: List[Dict[str, Any]]) -> Tuple[Dict[str, Entity], Dict[str, Relationship], List[Evidence]]:
        ...
        for ev in events:
            ...
            ev_data = ev.get('data', '')
            ev_hash = ev.get('hash')
            ev_module = ev.get('module', 'spiderfoot')
...
class ExposureEngine:
    """Master orchestrator for the SpiderFoot Exposure Intelligence Engine."""

    def __init__(self, dbh):
        self.dbh = dbh
        self.log = logging.getLogger("spiderfoot.exposure")
