...
class sfp_ripe(ShadowTracePlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "BGP_AS_OWNER":
            ...
            for netblock in netblocks:
                ...
                if ":" in netblock:
                    evt = ShadowTraceEvent("NETBLOCKV6_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = ShadowTraceEvent("NETBLOCK_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)

            evt = ShadowTraceEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if eventName.startswith("NETBLOCK"):
            ...
            if eventName in ["NETBLOCK_OWNER", "NETBLOCKV6_OWNER"] and self.ownsAs(asn):
                asevt = ShadowTraceEvent("BGP_AS_OWNER", asn, self.__name__, event)
                self.notifyListeners(asevt)
                evt = ShadowTraceEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
                self.notifyListeners(evt)
            else:
                asevt = ShadowTraceEvent("BGP_AS_MEMBER", asn, self.__name__, event)
                self.notifyListeners(asevt)
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            ...
            if ":" in prefix:
                evt = ShadowTraceEvent("NETBLOCKV6_" + relationship, prefix, self.__name__, event)
            else:
                evt = ShadowTraceEvent("NETBLOCK_" + relationship, prefix, self.__name__, event)
            self.notifyListeners(evt)
