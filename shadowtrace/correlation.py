...
class ShadowTraceCorrelator:
    """ShadowTrace correlation capabilities.

    Todo:
        Make the rule checking per analysis method
    """

    log = logging.getLogger("shadowtrace.correlator")
    dbh = None
    scanId = None
    types = None
    rules = list()
    ...
    def __init__(self, dbh: ShadowTraceDb, ruleset: dict, scanId: str = None) -> None:
        """Initialize ShadowTrace correlator engine with scan ID and ruleset.
        ...
        if not isinstance(dbh, ShadowTraceDb):
            raise TypeError(f"dbh is {type(dbh)}; expected ShadowTraceDb()")
    ...
    def build_db_criteria(self, matchrule: dict) -> dict:
        ...
        Returns:
            dict: criteria to be used with ShadowTraceDb.scanResultEvent()
