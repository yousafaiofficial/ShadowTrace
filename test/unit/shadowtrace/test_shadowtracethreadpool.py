...
from shadowtrace import ShadowTraceThreadPool


@pytest.mark.usefixtures
class TestShadowTraceThreadPool(unittest.TestCase):
    """
    Test ShadowTrace
    """
    ...
    def test_threadPool(self):
        ...
        # Example 1: using map()
        with ShadowTraceThreadPool(threads) as pool:
            map_results = sorted(
                list(pool.map(
                    callback,
                    iterable,
        ...
        pool = ShadowTraceThreadPool(threads)
        pool.start()
        for i in iterable2:
