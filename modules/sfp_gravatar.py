# -------------------------------------------------------------------------------
# Name:        sfp_gravatar
# Purpose:     ShadowTrace plug-in to search Gravatar API for an email address
...
class sfp_gravatar(ShadowTracePlugin):
    ...
    def handleEvent(self, event):
        ...
        if data.get('preferredUsername') is not None:
            ...
            evt = ShadowTraceEvent("USERNAME", un, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if data.get('name') is not None:
            ...
            for name in names:
                ...
                if full_name:
                    evt = ShadowTraceEvent("RAW_RIR_DATA", f"Possible full name: {full_name}", self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if data.get('phoneNumbers') is not None:
            for number in data.get('phoneNumbers'):
                if number.get('value') is not None:
                    evt = ShadowTraceEvent("PHONE_NUMBER", number.get('value'), self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if data.get('emails') is not None:
            for email in data.get('emails'):
                ...
                if not em:
                    continue
                if ShadowTraceHelpers.validEmail(em) and em != eventData:
                    if em.split("@")[0] in self.opts['_genericusers'].split(","):
                        evttype = "EMAILADDR_GENERIC"
                    else:
                        evttype = "EMAILADDR"

                    evt = ShadowTraceEvent(evttype, em, self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if data.get('ims') is not None:
            for im in data.get('ims'):
                ...
                evt = ShadowTraceEvent("ACCOUNT_EXTERNAL_OWNED", t, self.__name__, event)
                self.notifyListeners(evt)
                if v not in self.reportedUsers:
                    evt = ShadowTraceEvent("USERNAME", v, self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if data.get('accounts') is not None:
            for account in data.get('accounts'):
                ...
                if platform is not None and url is not None:
                    ...
                    evt = ShadowTraceEvent("SOCIAL_MEDIA", t, self.__name__, event)
                    self.notifyListeners(evt)
