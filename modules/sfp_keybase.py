...
# -------------------------------------------------------------------------------
# Name:        sfp_keybase
# Purpose:     Shadowtrace plugin to query KeyBase API
...
class sfp_keybase(ShadowTracePlugin):
    ...
    def handleEvent(self, event) -> None:
        ...
        for user in data:
            ...
            if eventName in ['LINKED_URL_EXTERNAL', 'DOMAIN_NAME']:
                ...
                source_event = ShadowTraceEvent("USERNAME", username, self.__name__, event)
                self.notifyListeners(source_event)
                self.results[username] = True
            else:
                source_event = event

            evt = ShadowTraceEvent("RAW_RIR_DATA", str(user), self.__name__, source_event)
            ...
            if profile:
                ...
                if fullName:
                    evt = ShadowTraceEvent("RAW_RIR_DATA", f"Possible full name: {fullName}", self.__name__, source_event)
                    self.notifyListeners(evt)

                # Get and report location of user
                location = profile.get('location')
                if location:
                    evt = ShadowTraceEvent("GEOINFO", location, self.__name__, source_event)
                    self.notifyListeners(evt)
                ...
                if socialMediaData:
                    for socialMedia in socialMediaData:
                        ...
                        if socialMediaSite and socialMediaURL:
                            ...
                            evt = ShadowTraceEvent("SOCIAL_MEDIA", socialMedia, self.__name__, source_event)
                            self.notifyListeners(evt)
            ...
            if cryptoAddresses:
                ...
                if bitcoinAddresses:
                    for bitcoinAddress in bitcoinAddresses:
                        ...
                        evt = ShadowTraceEvent("BITCOIN_ADDRESS", btcAddress, self.__name__, source_event)
                        self.notifyListeners(evt)
            ...
            for pgpKey in pgpKeys:
                ...
                evt = ShadowTraceEvent("PGP_KEY", pgpKey, self.__name__, source_event)
                self.notifyListeners(evt)
