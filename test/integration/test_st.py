...
class TestSf(unittest.TestCase):
    ...
    def test_no_args_should_print_arg_l_required(self):
        out, err, code = self.execute([sys.executable, "st.py"])
        self.assertIn(b"ShadowTrace requires -l <ip>:<port> to start the web server. Try --help for guidance.", out)
        self.assertEqual(b"", err)
        self.assertEqual(255, code)

    def test_help_arg_should_print_help_and_exit(self):
        out, err, code = self.execute([sys.executable, "st.py", "-h"])
        self.assertIn(b"show this help message and exit", out)
        self.assertEqual(b"", err)
        self.assertEqual(0, code)

    def test_modules_arg_should_print_modules_and_exit(self):
        out, err, code = self.execute([sys.executable, "st.py", "-M"])
        self.assertIn(b"Modules available:", err)
        self.assertEqual(0, code)

    def test_types_arg_should_print_types_and_exit(self):
        out, err, code = self.execute([sys.executable, "st.py", "-T"])
    ...
    def test_l_arg_should_start_web_server(self):
        ...
        out, err, code = self.execute([sys.executable, "st.py", "-l", listen])
    ...
    def test_debug_arg_should_enable_and_print_debug_output(self):
        out, err, code = self.execute([sys.executable, "st.py", "-d", "-m", "example module", "-s", "shadowtrace.net"])
    ...
    def test_quiet_arg_should_hide_debug_output(self):
        out, err, code = self.execute([sys.executable, "st.py", "-q", "-m", "example module", "-s", "shadowtrace.net"])
        self.assertNotIn(b"[INFO]", err)
        self.assertEqual(0, code)

    def test_run_scan_invalid_target_should_exit(self):
        invalid_target = '.'
        out, err, code = self.execute([sys.executable, "st.py", "-s", invalid_target])
    ...
    def test_run_scan_with_modules_no_target_should_exit(self):
        out, err, code = self.execute([sys.executable, "st.py", "-m", ",".join(self.default_modules)])
    ...
    def test_run_scan_with_types_no_target_should_exit(self):
        out, err, code = self.execute([sys.executable, "st.py", "-t", ",".join(self.default_types)])
        self.assertIn(b"You must specify a target when running in scan mode", err)
        self.assertEqual(255, code)

    def test_run_scan_with_invalid_module_should_run_scan_and_exit(self):
        module = "invalid module"
        out, err, code = self.execute([sys.executable, "st.py", "-m", module, "-s", "shadowtrace.net"])
        self.assertIn(bytes(f"Failed to load module: {module}", 'utf-8'), err)
        self.assertEqual(0, code)

    def test_run_scan_with_invalid_type_should_exit(self):
        out, err, code = self.execute([sys.executable, "st.py", "-t", "invalid type", "-s", "shadowtrace.net"])
        self.assertIn(b"Based on your criteria, no modules were enabled", err)
        self.assertEqual(255, code)

    def test_run_scan_should_run_scan_and_exit(self):
        target = "shadowtrace.net"
        out, err, code = self.execute([sys.executable, "st.py", "-m", ",".join(self.default_modules), "-s", target])
    ...
    def test_run_scan_should_print_scan_result_and_exit(self):
        target = "shadowtrace.net"
        out, err, code = self.execute([sys.executable, "st.py", "-m", ",".join(self.default_modules), "-s", target, "-o", "csv"])
        self.assertIn(b"Scan completed with status FINISHED", err)
        self.assertEqual(0, code)
        ...
        expected_output = [
            "Source,Type,Data",
            "ShadowTrace UI,Internet Name,shadowtrace.net,shadowtrace.net\n",
            "ShadowTrace UI,Domain Name,shadowtrace.net,shadowtrace.net\n",
            "sfp_countryname,Country Name,shadowtrace.net,United States\n",
        ]
