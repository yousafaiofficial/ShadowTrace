...
# Purpose:      ShadowTrace plug-in for identifying potential Digital Ocean spaces
...
class sfp_digitaloceanspace(ShadowTracePlugin):
    ...
    def handleEvent(self, event):
        ...
        for b in ret:
            ...
            self.notifyListeners(evt)
            if bucket[2] != "0":
                evt = ShadowTraceEvent("CLOUD_STORAGE_BUCKET_OPEN", bucket[0] + ":" + bucket[1] + ": " + bucket[2] + " files found.",
                                      self.__name__, evt)
                self.notifyListeners(evt)
