...
from shadowtrace import ShadowTraceDb, ShadowTraceEvent, ShadowTracePlugin, ShadowTraceTarget, ShadowTraceHelpers, ShadowTraceThreadPool, ShadowTraceCorrelator, logger
from shadowtrace.exposure import ExposureEngine


def startShadowTraceScanner(loggingQueue, *args, **kwargs):
    logger.logWorkerSetup(loggingQueue)
    return ShadowTraceScanner(*args, **kwargs)


class ShadowTraceScanner():
    """ShadowTraceScanner object.
    ...
    def __init__(self, scanName: str, scanId: str, targetValue: str, targetType: str, moduleList: list, globalOpts: dict, start: bool = True) -> None:
        """Initialize ShadowTraceScanner object.
        ...
        self.__dbh = ShadowTraceDb(self.__config)
        ...
        self.__sf = ShadowTrace(self.__config)
        ...
        if scanId:
            ...
            self.__scanId = ShadowTraceHelpers.genScanInstanceId()
        ...
        # Create our target
        try:
            self.__target = ShadowTraceTarget(self.__targetValue, self.__targetType)
        ...
        self.__sharedThreadPool = ShadowTraceThreadPool(threads=self.__config.get("_maxthreads", 3), name='sharedThreadPool')
    ...
    def __startScan(self) -> None:
        ...
        try:
            ...
            psMod = ShadowTracePlugin()
            psMod.__name__ = "ShadowTrace UI"
            ...
            rootEvent = ShadowTraceEvent("ROOT", self.__targetValue, "", None)
            psMod.notifyListeners(rootEvent)
            firstEvent = ShadowTraceEvent(self.__targetType, self.__targetValue,
                                         "ShadowTrace UI", rootEvent)
            ...
            if self.__targetType == 'INTERNET_NAME' and self.__sf.isDomain(self.__targetValue, self.__config['_internettlds']):
                firstEvent = ShadowTraceEvent('DOMAIN_NAME', self.__targetValue, "ShadowTrace UI", rootEvent)
                psMod.notifyListeners(firstEvent)
    ...
    def runCorrelations(self) -> None:
        ...
        corr = ShadowTraceCorrelator(self.__dbh, ruleset, self.__scanId)
    ...
    def waitForThreads(self) -> None:
        ...
        try:
            ...
            while True:
                ...
                if not isinstance(sfEvent, ShadowTraceEvent):
                    raise TypeError(f"sfEvent is {type(sfEvent)}; expected ShadowTraceEvent")
