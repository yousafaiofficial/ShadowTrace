...
from shadowtrace import ShadowTraceDb, ShadowTraceHelpers


class ShadowTraceSqliteLogHandler(logging.Handler):
    ...
    def makeDbh(self) -> None:
        """TBD."""
        self.dbh = ShadowTraceDb(self.opts)


def logListenerSetup(loggingQueue, opts: dict = None) -> 'logging.handlers.QueueListener':
    """Create and start a ShadowTrace log listener in its own thread.
    ...
    Args:
        ...
        opts (dict): ShadowTrace config

    Returns:
        shadowTraceLogListener (logging.handlers.QueueListener): Log listener
    """
    if opts is None:
        opts = dict()
    ...
    log_dir = ShadowTraceHelpers.logPath()
    debug_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/shadowtrace.debug.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    error_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/shadowtrace.error.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    if doLogging and opts is not None:
        sqlite_handler = ShadowTraceSqliteLogHandler(opts)
        sqlite_handler.setLevel(logLevel)
        sqlite_handler.setFormatter(log_format)
        handlers.append(sqlite_handler)
    shadowTraceLogListener = QueueListener(loggingQueue, *handlers)
    shadowTraceLogListener.start()
    atexit.register(stop_listener, shadowTraceLogListener)
    return shadowTraceLogListener


def logWorkerSetup(loggingQueue) -> 'logging.Logger':
    """Root ShadowTrace logger.
    ...
    """
    log = logging.getLogger("shadowtrace")
