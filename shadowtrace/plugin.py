...
from .threadpool import ShadowTraceThreadPool
...
class ShadowTracePluginLogger(logging.Logger):
    """Used only in ShadowTracePlugin to prevent modules
...
class ShadowTracePlugin():
    """ShadowTracePlugin module object
    ...
    Attributes:
        ...
        _currentEvent (ShadowTraceEvent): Current event being processed
    ...
    __scanId__ = None
    # (only used in ShadowTrace HX) tracking of data sources
    __dataSource__ = None
    ...
    # ShadowTrace object, set in each module's setup() function
    sf = None
    ...
    def log(self):
        if self._log is None:
            logging.setLoggerClass(ShadowTracePluginLogger)  # temporarily set logger class
            self._log = logging.getLogger(f"shadowtrace.{self.__name__}")  # init ShadowTracePluginLogger
    ...
    def setup(self, sf, userOpts: dict = {}) -> None:
        ...
        Args:
            sf (ShadowTrace): ShadowTrace object
            userOpts (dict): TBD
        """
        pass
    ...
    def setTarget(self, target) -> None:
        ...
        Args:
            target (ShadowTraceTarget): target

        Raises:
            TypeError: target argument was invalid type
        """
        from shadowtrace import ShadowTraceTarget

        if not isinstance(target, ShadowTraceTarget):
            raise TypeError(f"target is {type(target)}; expected ShadowTraceTarget")
    ...
    def setDbh(self, dbh) -> None:
        ...
        Args:
            dbh (ShadowTraceDb): database handle
        """
        self.__sfdb__ = dbh
    ...
    def tempStorage(self) -> dict:
        ...
        Note:
            Required for ShadowTrace HX compatibility of modules.
    ...
    def notifyListeners(self, sfEvent) -> None:
        ...
        Args:
            sfEvent (ShadowTraceEvent): event

        Raises:
            TypeError: sfEvent argument was invalid type
        """

        from shadowtrace import ShadowTraceEvent
    ...
    def threadWorker(self) -> None:
        try:
            ...
            self.setDbh(ShadowTraceDb(self.opts))
