...
# Purpose:      ShadowTrace plug-in to search HackerTarget.com for hosts sharing
...
class sfp_hackertarget(ShadowTracePlugin):
    ...
    def handleEvent(self, event):
        ...
        for ip in qrylist:
                ...
                if h not in myres and h != ip:
                    ...
                    if self.cohostcount < self.opts['maxcohost']:
                        ...
                        if eventName == "NETBLOCK_OWNER":
                            ...
                            evt = ShadowTraceEvent("CO_HOSTED_SITE", h.lower(), self.__name__, ipe)
                            self.notifyListeners(evt)
                        else:
                            evt = ShadowTraceEvent("CO_HOSTED_SITE", h.lower(), self.__name__, event)
                            self.notifyListeners(evt)
            ...
            if eventName == 'NETBLOCK_OWNER':
                pevent = ShadowTraceEvent("IP_ADDRESS", ip, self.__name__, event)
            ...
            if self.opts.get('http_headers', True):
                ...
                if http_headers is not None:
                    e = ShadowTraceEvent('WEBSERVER_HTTPHEADERS', json.dumps(http_headers), self.__name__, pevent)
                    e.actualSource = ip
