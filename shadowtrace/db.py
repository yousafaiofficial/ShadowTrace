...
class ShadowTraceDb:
    ...
    # Queries for creating the ShadowTrace database
    createSchemaQueries = [
    ...
    eventDetails = [
        ['ROOT', 'Internal ShadowTrace Root event', 1, 'INTERNAL'],
        ...
        with self.dbhLock:
            try:
                ...
                try:
                    ...
                    raise IOError("Tried to set up the ShadowTrace database schema, but failed") from e
    ...
    def scanEventStore(self, instanceId: str, sfEvent, truncateSize: int = 0) -> None:
        ...
        Args:
            instanceId (str): scan instance ID
            sfEvent (ShadowTraceEvent): event to be stored in the database
        ...
        if not isinstance(sfEvent, ShadowTraceEvent):
            raise TypeError(f"sfEvent is {type(sfEvent)}; expected ShadowTraceEvent()") from None
        ...
        if not isinstance(sfEvent.sourceEvent, ShadowTraceEvent) and sfEvent.eventType != "ROOT":
