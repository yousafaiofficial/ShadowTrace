...
3. **Evidence Engine**: Maintains data provenance (`Observed` vs `Inferred` vs `User Confirmed` mode, source module, timestamp, raw payload).
4. **Explainable Risk Engine**: Provides a 0-100 total exposure score breakdown with explicit additive factors (+25 credential leak, +20 repo leak) and "Why This Matters" natural language context.
5. **Interactive Attack Path Graph**: Cytoscape.js interactive canvas with node type icons, confidence threshold filtering, entity search, and Side Inspector Panel.
6. **Actionable Remediation Engine**: Translates findings directly into `EXPOSURE → IMPACT → RECOMMENDATION → PRIORITY` remediation steps.
