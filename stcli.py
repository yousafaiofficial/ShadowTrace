...
# -------------------------------------------------------------------------------
# Name:        sfcli
# Purpose:     Command Line Interface for ShadowTrace.
#
...
COPYRIGHT_INFO = "               by Steve Micallef | @shadowtrace\n"
...
class ShadowTraceCli(cmd.Cmd):
    ...
    # Make a request to the ShadowTrace server
    def request(self, url, post=None):
        ...
        headers = {
            "User-agent": "ShadowTrace-CLI/" + self.version,
    ...
    def do_ping(self, line):
        """ping
        Ping the ShadowTrace server to ensure it's responding."""
    ...
    # List all ShadowTrace correlation rules
    ...
    # List all ShadowTrace data element types.
    ...
    def do_load(self, line):
        """load <file>
        Execute ShadowTrace CLI commands found in <file>."""
        pass
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        ...
        helpmap = [
            ...
            ["exit", "Exit the ShadowTrace CLI (won't impact running scans)."],
            ["ping", "Test connectivity to the ShadowTrace server."],
            ...
            ["query", "Run SQL against the ShadowTrace SQLite database."],
    ...
    def do_set(self, line):
        """set [opt [= <val>]]
        Set a configuration variable in ShadowTrace."""
        ...
        if not d:
            self.edprint("Unable to obtain ShadowTrace server-side config.")
            return
        ...
        j = json.loads(d)
        if j[0] == "ERROR":
            self.edprint("Error fetching ShadowTrace server-side config.")
            return
        ...
        if val:
            ...
            if not d:
                self.edprint("Unable to set ShadowTrace server-side config.")
                return

            j = json.loads(d)
            if j[0] == "ERROR":
                self.edprint(f"Error setting ShadowTrace server-side config: {j[1]}")
                return
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description='ShadowTrace: Open Source Intelligence Automation.')
    p.add_argument("-d", "--debug", help="Enable debug output.", action='store_true')
    p.add_argument("-s", metavar="URL", type=str, help="Connect to ShadowTrace server on URL. By default, a connection to http://127.0.0.1:5001 will be attempted.")
    p.add_argument("-u", metavar="USER", type=str, help="Username to authenticate to ShadowTrace server.")
    p.add_argument("-p", metavar="PASS", type=str, help="Password to authenticate to ShadowTrace server. Consider using -P PASSFILE instead so that your password isn't visible in your shell history or in process lists!")
    p.add_argument("-P", metavar="PASSFILE", type=str, help="File containing password to authenticate to ShadowTrace server. Ensure permissions on the file are set appropriately!")
    p.add_argument("-e", metavar="FILE", type=str, help="Execute commands from FILE.")
    p.add_argument("-l", metavar="FILE", type=str, help="Log command history to FILE. By default, history is stored to ~/.shadowtrace_history unless disabled with -n.")
    ...
    if args.l:
        ...
        try:
            s.ownopts['cli.history_file'] = expanduser("~") + "/.shadowtrace_history"
        except BaseException as e:
            s.dprint(f"Failed to set 'cli.history_file': {e}")
            s.dprint("Using '.shadowtrace_history' in working directory")
            s.ownopts['cli.history_file'] = ".shadowtrace_history"
    if args.o:
