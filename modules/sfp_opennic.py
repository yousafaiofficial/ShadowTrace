...
# -------------------------------------------------------------------------------
# Name:        sfp_opnenic
# Purpose:     ShadowTrace plug-in for resolving host names on the OpenNIC
...
class sfp_opennic(ShadowTracePlugin):
        ...
        for addr in set(addrs):
            if self.sf.validIP(addr):
                if affiliate and not self.getTarget().matches(addr, includeParents=True):
                    evt = ShadowTraceEvent("AFFILIATE_IPADDR", addr, self.__name__, event)
                else:
                ...
                if affiliate and not self.getTarget().matches(addr, includeParents=True):
                    evt = ShadowTraceEvent("AFFILIATE_IPV6_ADDRESS", addr, self.__name__, event)
                else:
                    evt = ShadowTraceEvent("IPV6_ADDRESS", addr, self.__name__, event)
