...
from stlib import ShadowTrace

from stscan import startShadowTraceScanner

from shadowtrace import ShadowTraceDb
from shadowtrace import ShadowTraceHelpers
from shadowtrace import __version__
from shadowtrace.logger import logListenerSetup, logWorkerSetup

mp.set_start_method("spawn", force=True)


class ShadowTraceWebUi:
    """ShadowTrace web interface."""
    ...
    def __init__(self: 'ShadowTraceWebUi', web_config: dict, config: dict, loggingQueue: 'logging.handlers.QueueListener' = None) -> None:
        """Initialize web server.

        Args:
            web_config (dict): config settings for web interface (interface, port, root path)
            config (dict): ShadowTrace config
            loggingQueue: TBD
        ...
        dbh = ShadowTraceDb(self.defaultConfig, init=True)
        sf = ShadowTrace(self.defaultConfig)
        ...
        self.log = logging.getLogger(f"shadowtrace.{__name__}")
    ...
    def error_page(self: 'ShadowTraceWebUi') -> None:
        """Error page."""
    ...
    def error_page_401(self: 'ShadowTraceWebUi', status: str, message: str, traceback: str, version: str) -> str:
    ...
    def error_page_404(self: 'ShadowTraceWebUi', status: str, message: str, traceback: str, version: str) -> str:
        ...
        """
        templ = Template(filename='shadowtrace/templates/error.tmpl', lookup=self.lookup)
        return templ.render(message='Not Found', docroot=self.docroot, status=status, version=__version__)

    def jsonify_error(self: 'ShadowTraceWebUi', status: str, message: str) -> dict:
    ...
    def error(self: 'ShadowTraceWebUi', message: str) -> None:
    ...
    def cleanUserInput(self: 'ShadowTraceWebUi', inputList: list) -> list:
    ...
    def searchBase(self: 'ShadowTraceWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search.
    ...
    def buildExcel(self: 'ShadowTraceWebUi', data: list, columnNames: list, sheetNameIndex: int = 0) -> str:
    ...
    @cherrypy.expose
    def scanexportlogs(self: 'ShadowTraceWebUi', id: str, dialect: str = "excel") -> bytes:
        """Get scan log
        ...
        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename=ShadowTrace-{id}.log.csv"
        cherrypy.response.headers['Content-Type'] = "application/csv"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return fileobj.getvalue().encode('utf-8')

    @cherrypy.expose
    def scancorrelationsexport(self: 'ShadowTraceWebUi', id: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if scan_name:
                fname = f"{scan_name}-ShadowTrace-correlations.xlxs"
            else:
                fname = "ShadowTrace-correlations.xlxs"
        ...
        if filetype.lower() == 'csv':
            ...
            if scan_name:
                fname = f"{scan_name}-ShadowTrace-correlations.csv"
            else:
                fname = "ShadowTrace-correlations.csv"
    ...
    @cherrypy.expose
    def scaneventresultexport(self: 'ShadowTraceWebUi', id: str, type: str, filetype: str = "csv", dialect: str = "excel") -> str:
    ...
    @cherrypy.expose
    def scaneventresultexportmulti(self: 'ShadowTraceWebUi', ids: str, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-ShadowTrace.xlsx"
        ...
        if filetype.lower() == 'csv':
            ...
            if len(ids.split(',')) > 1 or scan_name == "":
                ...
                fname = scan_name + "-ShadowTrace.csv"
    ...
    @cherrypy.expose
    def scansearchresultexport(self: 'ShadowTraceWebUi', id: str, eventType: str = None, value: str = None, filetype: str = "csv", dialect: str = "excel") -> str:
        ...
        if filetype.lower() in ["xlsx", "excel"]:
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=ShadowTrace.xlsx"
            cherrypy.response.headers['Content-Type'] = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return self.buildExcel(rows, ["Updated", "Type", "Module", "Source",
                                   "F/P", "Data"], sheetNameIndex=1)
        ...
        if filetype.lower() == 'csv':
            ...
            cherrypy.response.headers['Content-Disposition'] = "attachment; filename=ShadowTrace.csv"
            cherrypy.response.headers['Content-Type'] = "application/csv"
            cherrypy.response.headers['Pragma'] = "no-cache"
            return fileobj.getvalue().encode('utf-8')
    ...
    @cherrypy.expose
    def scanexportjsonmulti(self: 'ShadowTraceWebUi', ids: str) -> str:
        ...
        if len(ids.split(',')) > 1 or scan_name == "":
            fname = "ShadowTrace.json"
        else:
            fname = scan_name + "-ShadowTrace.json"
    ...
    @cherrypy.expose
    def scanviz(self: 'ShadowTraceWebUi', id: str, gexf: str = "0") -> str:
        ...
        if gexf == "0":
            return ShadowTraceHelpers.buildGraphJson([root], data)

        if not scan_name:
            fname = "ShadowTrace.gexf"
        else:
            fname = scan_name + "ShadowTrace.gexf"

        cherrypy.response.headers['Content-Disposition'] = f"attachment; filename={fname}"
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return ShadowTraceHelpers.buildGraphGexf([root], "ShadowTrace Export", data)

    @cherrypy.expose
    def scanvizmulti(self: 'ShadowTraceWebUi', ids: str, gexf: str = "1") -> str:
        ...
        cherrypy.response.headers['Content-Type'] = "application/gexf"
        cherrypy.response.headers['Pragma'] = "no-cache"
        return ShadowTraceHelpers.buildGraphGexf(roots, "ShadowTrace Export", data)

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanopts(self: 'ShadowTraceWebUi', id: str) -> dict:
    ...
    @cherrypy.expose
    def rerunscan(self: 'ShadowTraceWebUi', id: str) -> None:
        """Rerun a scan.
        ...
        targetType = ShadowTraceHelpers.targetTypeFromString(scantarget)
        if not targetType:
            ...
            # target type again.
            targetType = ShadowTraceHelpers.targetTypeFromString(f'"{scantarget}"')

        if targetType not in ["HUMAN_NAME", "BITCOIN_ADDRESS"]:
            scantarget = scantarget.lower()

        # Start running a new scan
        scanId = ShadowTraceHelpers.genScanInstanceId()
        try:
            p = mp.Process(target=startShadowTraceScanner, args=(self.loggingQueue, scanname, scanId, scantarget, targetType, modlist, cfg))
            p.daemon = True
            p.start()
        except Exception as e:
    ...
    @cherrypy.expose
    def rerunscanmulti(self: 'ShadowTraceWebUi', ids: str) -> str:
        """Rerun scans.
        ...
        for id in ids.split(","):
            ...
            if targetType is None:
                ...
                return self.error("Invalid target type. Could not recognize it as a target ShadowTrace supports.")
        ...
        templ = Template(filename='shadowtrace/templates/scanlist.tmpl', lookup=self.lookup)
        return templ.render(rerunscans=True, docroot=self.docroot, pageid="SCANLIST", version=__version__)

    @cherrypy.expose
    def newscan(self: 'ShadowTraceWebUi') -> str:
        ...
        templ = Template(filename='shadowtrace/templates/newscan.tmpl', lookup=self.lookup)
        return templ.render(pageid='NEWSCAN', types=types, docroot=self.docroot,
                            modules=self.config['__modules__'], scanname="",
                            selectedmods="", scantarget="", version=__version__)

    @cherrypy.expose
    def clonescan(self: 'ShadowTraceWebUi', id: str) -> str:
    ...
    @cherrypy.expose
    def index(self: 'ShadowTraceWebUi') -> str:
        """Show scan list page.
    ...
    @cherrypy.expose
    def scaninfo(self: 'ShadowTraceWebUi', id: str) -> str:
        ...
        templ = Template(filename='shadowtrace/templates/scaninfo.tmpl', lookup=self.lookup, input_encoding='utf-8')
        return templ.render(id=id, name=html.escape(res[0]), status=res[5], docroot=self.docroot, version=__version__,
                            pageid="SCANLIST")

    @cherrypy.expose
    def exposuregraph(self: 'ShadowTraceWebUi', id: str) -> str:
        ...
        templ = Template(filename='shadowtrace/templates/exposure.tmpl', lookup=self.lookup, input_encoding='utf-8')
        return templ.render(id=id, name=html.escape(res[0]), status=res[5], docroot=self.docroot, version=__version__, pageid="EXPOSURE")

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def exposureinfo(self: 'ShadowTraceWebUi', id: str) -> dict:
        ...
        if not summary and res[5] in ["FINISHED", "COMPLETED"]:
            from shadowtrace.exposure import ExposureEngine
    ...
    def opts(self: 'ShadowTraceWebUi', updated: str = None) -> str:
        ...
        """
        templ = Template(filename='shadowtrace/templates/opts.tmpl', lookup=self.lookup)
    ...
    @cherrypy.expose
    def optsexport(self: 'ShadowTraceWebUi', pattern: str = None) -> str:
        ...
        cherrypy.response.headers['Content-Disposition'] = 'attachment; filename="ShadowTrace.cfg"'
        cherrypy.response.headers['Content-Type'] = "text/plain"
        return content

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def optsraw(self: 'ShadowTraceWebUi') -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scandelete(self: 'ShadowTraceWebUi', id: str) -> str:
        """Delete scan(s).
    ...
    @cherrypy.expose
    def savesettings(self: 'ShadowTraceWebUi', allopts: str, token: str, configFile: 'cherrypy._cpreqbody.Part' = None) -> None:
        ...
        if configFile and configFile.file:
            try:
                ...
                return self.error(f"Failed to parse input file. Was it generated from ShadowTrace? ({e})")
    ...
    @cherrypy.expose
    def savesettingsraw(self: 'ShadowTraceWebUi', allopts: str, token: str) -> str:
        """Save settings, also used to completely reset them to default.
    ...
    def reset_settings(self: 'ShadowTraceWebUi') -> bool:
    ...
    @cherrypy.expose
    def resultsetfp(self: 'ShadowTraceWebUi', id: str, resultids: str, fp: str) -> str:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def eventtypes(self: 'ShadowTraceWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def modules(self: 'ShadowTraceWebUi') -> list:
        """List all modules.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def correlationrules(self: 'ShadowTraceWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def ping(self: 'ShadowTraceWebUi') -> list:
        """For the CLI to test connectivity to this server.

        Returns:
            list: ShadowTrace version as JSON
        """
        return ["SUCCESS", __version__]

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def query(self: 'ShadowTraceWebUi', query: str) -> str:
    ...
    @cherrypy.expose
    def startscan(self: 'ShadowTraceWebUi', scanname: str, scantarget: str, modulelist: str, typelist: str, usecase: str) -> str:
        """Initiate a scan.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def stopscan(self: 'ShadowTraceWebUi', id: str) -> str:
        """Stop a scan.
        ...
        for scan_id in ids:
            ...
            if scan_status != "RUNNING" and scan_status != "STARTING":
                return self.jsonify_error('400', f"The running scan is currently in the state '{scan_status}', please try again later or restart ShadowTrace.")
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlog(self: 'ShadowTraceWebUi', id: str, limit: str = None, rowId: str = None, reverse: str = None) -> list:
        """Scan log data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanerrors(self: 'ShadowTraceWebUi', id: str, limit: str = None) -> list:
        """Scan error data.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanlist(self: 'ShadowTraceWebUi') -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanstatus(self: 'ShadowTraceWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scansummary(self: 'ShadowTraceWebUi', id: str, by: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scancorrelations(self: 'ShadowTraceWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresults(self: 'ShadowTraceWebUi', id: str, eventType: str = None, filterfp: bool = False, correlationId: str = None) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scaneventresultsunique(self: 'ShadowTraceWebUi', id: str, eventType: str, filterfp: bool = False) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def search(self: 'ShadowTraceWebUi', id: str = None, eventType: str = None, value: str = None) -> list:
        """Search scans.
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanhistory(self: 'ShadowTraceWebUi', id: str) -> list:
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out()
    def scanelementtypediscovery(self: 'ShadowTraceWebUi', id: str, eventType: str) -> dict:
        ...
        del pc['ROOT']
        retdata['tree'] = ShadowTraceHelpers.dataParentChildToTree(pc)
